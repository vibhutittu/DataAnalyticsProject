...
X_train<- X_train[-(which(X_train$country_destination == "US"))]
...
train_rpart <- fread("./train_gbm.csv")
test_rpart <- fread("./test_gbm.csv")
...
sessions_action_detail <- sessions_action_detail[, ":="(coupon = ceiling((apply_coupon+apply_coupon_click+apply_coupon_click_success+apply_coupon_error+coupon_code_click+coupon_field_focus)/10),
                                                        cancel = ceiling((cancellation_policies+cancellation_policies+cancellation_policy_click)/10),
                                                        guest = ceiling((guest_cancellation+guest_itinerary+guest_receipt)/10),
                                                        host = ceiling((host_guarantee+host_home)/10),
...
sessions_action <- sessions_action[, ":="(
                                          google_translate = ceiling((ajax_google_translate)/10),
...
# model building
set.seed(23)
model_xgb <- xgboost(as.matrix(X_train), target, objective="multi:softprob", num_class=10, nrounds=50, eta=0.05, max_depth=5, subsample=0.9, colsample_bytree=0.5, min_child_weight=5, eval_metric='mlogloss')

# model scoring
pred <- predict(model_xgb, as.matrix(X_test))
pred_matrix <- as.data.frame(matrix(pred, nrow(X_test), 10, byrow=T))
...
submit <- merge(submit, data.frame("value"=seq(1,9),"country"=c("AU","CA","DE","ES","FR","GB","IT","NL","other")))

X_train_merged <- merge(submit, t1, by.x = "id", by.y = "id")
X_train_merged <- X_train_merged[-(which(X_train_merged$country_destination =="PT")),]

X_train_merged$country_destination <- as.factor(X_train_merged$country_destination)
code <- read.csv("t.csv")
X_train_merged$actual <- code$t

cm2<-confusionMatrix(X_train_merged$actual,X_train_merged$value,mode="everything")
                     
                     

importance_matrix <- xgb.importance(names, model=model_xgb)[1:20] # View top 20 most important features
